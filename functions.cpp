...
int openPosition(struct minePosition** mineMatrix, int nRows, int nCols, int row, int col)
{
	...
	if (mineMatrix[row][col].neighborMines == -1){
		...
		mineMatrix[row][col].open = true;
		int count = countNeighborMines(mineMatrix, nRows, nCols, row, col);
		
		if(count>0){
			
			mineMatrix[row][col].neighborMines = count;
		}
		else if ((count)==0){
					for(int i =row-1; i < row+2; i++ ){
						for(int j = col-1 ; j < col+2; j++){
							if((i>=0) && (i<nRows) && (j>=0)&& (j< nCols)&&(!((j ==row) && (i == col)))){
								
							if((countNeighborMines(mineMatrix, nRows, nCols, i, j)==0)&&(!(mineMatrix[i][j].open))){
								openPosition( mineMatrix,  nRows,  nCols, i,  j);}
								
							else if ((countNeighborMines(mineMatrix, nRows, nCols, i, j)>0)){
								mineMatrix[i][j].open = true;
								mineMatrix[row][col].neighborMines =countNeighborMines(mineMatrix, nRows, nCols, i, j);
							}}}}}}
  return 0; 
}
